    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        lambda_get_data = lambda_.Function(
            ...
            code=lambda_.Code.from_bucket(bucket=source_bucket,
                                          key='scripts/get_data_api.zip'),
            layers=[requestLayer])
        lambda_get_data.role.add_managed_policy(
        ...
        #Crawler
        glue_source_crawler = glue.CfnCrawler(
            self,
            source_crawler_name,
        ...
        #create glue db
        glue_db_neighborhood = glue2.Database(
            self,
            glue_db_name_neighborhood,
            database_name=glue_db_name_neighborhood)

        #Crawler
        glue_source_crawler_neighborhood = glue.CfnCrawler(
            self,
            source_crawler_name_neighborhood,
        ...
        glue_processed_crawler = glue.CfnCrawler(
            self,
        ...
        glue_job_process_data = glue.CfnJob(
            self,
            glue_job_process,
            command=glue.CfnJob.JobCommandProperty(
                ...
                python_version='3',
                script_location=
                f's3://{source_bucket_name}/scripts/process_crime_data.py'),
        ...
        # Glue WF
        glue_crime_workflow = glue.CfnWorkflow(
            self,
            glue_process_crime_wf,
            name=glue_process_crime_wf,
        )

        # trigger from EventBidge when s3:PutItem in input folder or when lambda completes
        glue_initial_tgr = glue.CfnTrigger(
            self,
            glue_tgr_initial_name,
        ...
        glue_input_crawler_tgr = glue.CfnTrigger(
            self,
            'glue_input_crawler_tgr',
            ...
            predicate=glue.CfnTrigger.PredicateProperty(conditions=[
        ...
        glue_processed_crawler_tgr = glue.CfnTrigger(
            self,
            'glue_processed_crawler_tgr',
        ...
        event_process_source_file = events.CfnRule(
            self,
            'event_crime_wf_tgr',
            ...
            targets=[
                events.CfnRule.TargetProperty(
                    arn=
                    f'arn:aws:glue:us-east-1:896639149083:workflow/{glue_crime_workflow.name}',
                    # arn = glue_crime_workflow.get_att('arn').to_string(),
                    id=glue_process_crime_wf,
                    role_arn=
                    'arn:aws:iam::896639149083:role/service-role/Amazon_EventBridge_Invoke_Glue_143070630'
                )
            ])

        s3_trail = cloudtrail.Trail(
            self,
            "s3_trail",
            enable_file_validation=False,
        )
        s3_trail.add_s3_event_selector(
            s3_selector=[
                cloudtrail.S3EventSelector(bucket=source_bucket,
                                           object_prefix='input/')
            ],
